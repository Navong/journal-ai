        ...
        {!isLoading && reflection && onTogglePlayback && (
          <div className="absolute top-3 right-3 md:top-4 md:right-4 flex items-center gap-2">
            {isPlaying && (
              <div className="flex items-center gap-1 bg-white/80 backdrop-blur-sm rounded-full px-2 py-1 shadow-sm border border-stone-200">
                ...
                {onStop && (
                  <button
                    onClick={onStop}
                    className="text-[10px] text-stone-600 hover:text-rose-600 px-1 ml-1 border-l border-stone-300 pl-1"
                    title="Stop"
                  >
                    ⊗
                  </button>
            ...
            <button
              ...
              className={`transition-all p-2.5 md:p-2 rounded-full active:scale-90 ${isGeneratingVoice ? 'bg-stone-100 text-stone-300' : isPlaying ? 'text-emerald-600 bg-emerald-50' : 'text-stone-400 hover:text-emerald-600 hover:bg-stone-100'}`}
              title={isGeneratingVoice ? "Generating..." : isPaused ? "Resume" : isPlaying ? "Pause" : "Listen"}
            >
              {isGeneratingVoice ? (
